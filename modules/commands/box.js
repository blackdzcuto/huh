...
module.exports.run = async ({
  ...
  if (args[0] == "me") {
    if (args[1] == "admin") {
      ...
      else if (!global.config.ADMINBOT.includes(event.senderID)) api.sendMessage("Bạn không có quyền sử dụng lệnh này ?", event.threadID, event.messageID)
  ...
  if (args[0] == "admin") {
    ...
    if (!finddd) return api.sendMessage("» Bạn đéo phải quản trị viên box ?", event.threadID, event.messageID);
    if (!find) {
      api.sendMessage("» Không ném quản trị viên dùng cc ?", event.threadID, event.messageID)
    }
    if (!findd) {
