  ...
  var seconds = moment.tz("Asia/Ho_Chi_Minh").format("ss");
    ...
    else return api.sendMessage(`[ WARNING ] » Bạn đã có tài khoản trên hệ thống HaiChân Bank🏦`, threadID, messageID)
    }
    ...
    if (dataJson.some(i => i.senderID == senderID) == false) return api.sendMessage('[ WARNING ] » Người dùng chưa đăng kí sử dụng banking, banking register để đăng kí🏦', threadID, messageID)
        ...
        return api.sendMessage(`[ SUCCESS ] » Số tiền bạn đang gửi HaiChân Bank là: ${userMoney}$\n💷 Lãi: +${laisuat}% trong ${timeIM/60} phút`, threadID, messageID)
        }
    } 
    ...
    if (!dataJson.some(i => i.senderID == senderID)) {
        ...
        if(moneyy < moneyInput) return api.sendMessage(`[ WARNING ] » Số dư không đủ ${moneyInput} để gửi vào HaiChân Bank💰 `, threadID, messageID)
        var money = userData.money;
        userData.money = parseInt(money) + parseInt(moneyInput);
        writeFileSync(pathData, JSON.stringify(dataJson, null, 4), "utf-8");
        await Currencies.decreaseMoney(event.senderID, parseInt(moneyInput))
        
        return api.sendMessage(`[ SUCCESS ] » Bạn đã gửi ${moneyInput}$ vào HaiChân Bank\n💷 Lãi: +${laisuat}% trong ${timeIM/60} phút`, threadID, messageID)
        }
    }
    ...
    else return api.sendMessage(`=====🏦HaiChân BANK🏦=====\n\n[-r/register] - Đăng kí gửi tiền tại HaiChân Bank💹\n[check/coins] - Xem số tiền trong HaiChân Bank💳\n[gửi/send] - Gửi tiền vào HaiChân Bank💷\n[rút] - Rút tiền từ HaiChân Bank💰\n\n💲 Lãi suất hiện tại: +${laisuat}% trong ${timeIM/60} phút`, threadID, messageID)
    }
async function bank() {
