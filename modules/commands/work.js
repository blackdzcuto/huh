...
var coinscn = Math.floor(Math.random() * 400001) + 200; //random coins khi làm ở khu công nghiệp
var coinsdv = Math.floor(Math.random() * 800001) + 200; //random coins khi làm ở khu dịch vụ
var coinsmd = Math.floor(Math.random() * 400001) + 200; //random coins khi làm ở mỏ dầu
var coinsq = Math.floor(Math.random() * 600001) + 200; //random coins khi khai thác quặng
var coinsdd = Math.floor(Math.random() * 200001) + 200; //random coins khi đào đá
var coinsdd1 = Math.floor(Math.random() * 899901) + 200; //random coins khi đào đá
...
var rddd1 = ['Hoàng Văn Sơn', 'Hoàng Xuân Việt', 'Nguyễn Thị Luân', 'thằng ngu Nguyễn Văn An', '2 anh Tây Đào Tuấn Tiến và Đạt 09', 'Bà lão tóc xoăn mông tròn', 'con vợ Nguyễn Văn Đức', 'một con chó', 'Daddy Kiên Trần','Trần Đức Bo','1 con khỉ đột','bà cụ 90 tuổi','Cậu Vàng của Lão Hạc',]; //random công việc khi đào đá
    ...
    switch(handleReply.type) {
        case "choosee": {
            ...
            switch(event.body) {
                ...
                case "6": msg = `⚡️Bạn được ${work6} cho ${coinsdd1}$ nếu xxx 1 đêm, thế là bạn đồng ý ngay, bạn đúng là con phò hư hỏng🙂 :)))`; Currencies.increaseMoney(event.senderID, coinsdd1); break;
                case "7": msg = "⚡️Chưa update..."; break; //thêm case nếu muốn 
                default: break;
            };
            const choose = parseInt(event.body);
            if (isNaN(event.body)) return api.sendMessage("⚡️Vui lòng nhập 1a con số", event.threadID, event.messageID);
